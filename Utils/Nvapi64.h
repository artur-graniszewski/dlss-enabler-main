...
NvAPI_Status __cdecl NvAPI_GPU_GetArchInfo(NvPhysicalGpuHandle hPhysicalGpu, NV_GPU_ARCH_INFO* pGpuArchInfo)
{
	...
	if (!ctx.nvapi.isEmbeddedNvapiUsed && org_NvAPI_GPU_GetArchInfo) {
		...
		if (lastGpuReported == nullptr) {
			std::wstring arch = (ctx.realGpuArchitecture == NV_GPU_ARCHITECTURE_AD100 ? L"ada" : (ctx.realGpuArchitecture == NV_GPU_ARCHITECTURE_GA100 ? L"ampere" : L"turing or below"));
	...
	if (ctx.nvapi.isEmbeddedNvapiUsed || !org_NvAPI_GPU_GetArchInfo) {
			pGpuArchInfo->architecture = fakeArch;
			pGpuArchInfo->architecture_id = fakeArch;
...
NvAPI_Status __cdecl NvAPI_GPU_GetAllClockFrequencies(NvPhysicalGpuHandle hPhysicalGpu, NV_GPU_CLOCK_FREQUENCIES* pClkFreqs)
{
	...
	if (pClkFreqs->version != NV_GPU_CLOCK_FREQUENCIES_VER_1 
...
NvAPI_Status __cdecl NvAPI_DRS_GetSetting(NvDRSSessionHandle hSession, NvDRSProfileHandle hProfile, NvU32 settingId, NVDRS_SETTING* pSetting)
{
	...
	if (hSession != drsSession || hProfile != drsProfile || !pSetting) {
	...
	LOG_NVAPI_FUNCTION_CALL_AND_RETURN(NVAPI_SETTING_NOT_FOUND); // its enough on GTX 1060
	...
	case 0x10AFB76B:
		...
		wcscpy_s((wchar_t*)pSetting->wszCurrentValue, NVAPI_UNICODE_STRING_MAX, L"c:\\users\\admin\\desktop\\dlss\\test.log");
		break;
	case 0x10E41DF2:
	...
	case 0x10AFB764:
		...
		break;
	default:
		LOG_NVAPI_ERROR(L"Requested Unknown DRS setting with ID=" + std::to_wstring(settingId));
		LOG_NVAPI_FUNCTION_CALL_AND_RETURN(NVAPI_SETTING_NOT_FOUND);
	}

	LOG_NVAPI_WARNING(L"Requested DRS setting with ID=" + std::to_wstring(settingId) + L": " + settingName);
	...
	if (!org_NvAPI_D3D_Sleep) {
		...
		if (ctx.ngx.lastEvaluationTimeMsec > 0.0f) { // DLSSG is enabled....
			if (!OverdriveController::GetDynamicFrameGenerationEnabled() && (ctx.nvapi.isMockEnabled || !ctx.nvapi.isGenuineFileLoaded || ctx.nvapi.isEmbeddedNvapiUsed)) {
				ctx.reflex.realFpsLimit = ((double)ctx.reflex.desiredFpsLimit) / (ctx.ngx.framesGenerated + 1); // Only real Reflex is aware of extra frames generated by DLSSG
			}
		}
		...
		if (lastSleep != sleep) {
			...
			setSleepModeParams.bLowLatencyMode = OverdriveController::GetReflexEnabled() ? ctx.reflex.isOriginallyEnabled : 0;
			setSleepModeParams.bUseMarkersToOptimize = ctx.reflex.isMarkersOptimizationEnabled;
			setSleepModeParams.minimumIntervalUs = sleep;
			...
			if (status != NVAPI_OK) {
				...
				LOG_DEBUG(L"[RLFX] NvAPI_D3D_Sleep: setting minimumIntervalUs to " + std::to_wstring(sleep));
			}
		}
