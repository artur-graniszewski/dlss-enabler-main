...
namespace NVAPI
{
    std::wstring GetNvAPIFunctionName(unsigned int id)
    {
        struct NVAPI_INTERFACE_TABLE_INT { const char* func; unsigned int id; };
        struct NVAPI_INTERFACE_TABLE_INT nvapi_interface_map[] =
        {
            ...
            { "NvAPI_GPU_GetValuesFromInstalledINF", 0xf2400ab },
        ...
        for (const auto& entry : nvapi_interface_map) {
            if (entry.id == id) {
                return ToWideString(std::string(entry.func));
            }
        }

        std::wstringstream hexStream;
        hexStream << L"0x" << std::hex << id;
        std::wstring hexString = hexStream.str();

        auto result = L"Unknown function (" + hexString + L")";
        return result;
    }
}