...
namespace FpsMonitor
{
    ...
    static void OnPrePresent(IDXGISwapChain* pSwapChain, UINT& SyncInterval, UINT& Flags)
    {
        ...
        LatencyProbe::OnPrePresent(pSwapChain, SyncInterval);
    ...
    {
        // Recorded before the early-out: a failed Present is a data point, not
        // something to hide from the log.
        LatencyProbe::OnPostPresent(result);
    ...
    static void ProcessMarker(uint64_t frameId, uint32_t markerType, bool isAsync)
    {
        ...
        // Developer probe: record the raw marker timing regardless of whether
        // the NVAPI behind us is genuine or emulated.
        LatencyProbe::OnMarker(frameId, markerType, isAsync);
    ...
    {
        // Snapshot first, filter second. On the emulated path this buffer is
        // wrong in interesting ways and that is precisely what we want on file.
        LatencyProbe::OnGetLatency(pParams, result);
    ...
    void Init()
    {
        ...
        // Developer-only latency instrumentation (compiled out when
        // LATENCY_PROBE == 0). Must come up before the first marker arrives.
        LatencyProbe::Init();
    ...
    // Latency API
    //
    // Our own measurement comes first. It is derived from Reflex markers plus
    // GPU timestamps we take ourselves, so it does not depend on
    // NV_LATENCY_RESULT_PARAMS being populated and works identically whether the
    // NVAPI behind us is genuine or emulated:
    //
    //     without frame generation:  gpuEnd(overlay)      - simStart
    //     with frame generation:     gpuEnd(last subframe) - simStart
    //
    // The NVAPI-report path stays as a fallback for the case where our GPU
    // timestamps never come up (no D3D12 overlay submit, no frame-generation
    // dispatch). Note the report-derived figure sums phases that overlap -
    // gpuRender sits entirely inside osRenderQueue - so it overshoots by
    // roughly the GPU time plus the present time. It is a last resort, not a
    // second opinion.
    double GetAverageLatencyMs()
    {
        if (LatencyProbe::HasLatency())
        {
            const double ms = LatencyProbe::GetLatencyMs();
            if (ms > 0.0)
                return ms;
        }
        return g_AverageLatencyMs.load(std::memory_order_relaxed);
    }

    bool HasLatencyData()
    {
        if (LatencyProbe::HasLatency() && LatencyProbe::GetLatencyMs() > 0.0)
            return true;
