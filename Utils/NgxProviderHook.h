...
/**
    todo: 
    1. add all the NGX functions (VULKAN, CUDA, D3D11...)
    2. use GetProxy aliasing instead of detours (to implement missing function like Init_ProjectID)
...
namespace NGX
{
    NGX_MAKE_PROXY(NVSDK_NGX_VULKAN_Init,
        ...
        return ngxProvider->InitVulkan(InApplicationId, InApplicationDataPath, InInstance, InPD, InDevice, InGIPA, InGDPA, InFeatureInfo, InSDKVersion);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_VULKAN_Init_Ext,
        ...
        return ngxProvider->InitVulkanExt(InApplicationId, InApplicationDataPath, InInstance, InPD, InDevice, InSDKVersion, InFeatureInfo);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_VULKAN_Init_Ext2,
        ...
        return ngxProvider->InitVulkanExt2(InApplicationId, InApplicationDataPath, InInstance, InPD, InDevice, InGIPA, InGDPA, InSDKVersion, InFeatureInfo);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_VULKAN_CreateFeature, (VkCommandBuffer InCmdList, NVSDK_NGX_Feature InFeatureID, NVSDK_NGX_Parameter* InParameters, NVSDK_NGX_Handle** OutHandle)) {
        return ngxProvider->CreateVulkan(InCmdList, InFeatureID, InParameters, OutHandle);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_VULKAN_CreateFeature1, (const VkDevice InDevice, VkCommandBuffer InCmdList, NVSDK_NGX_Feature InFeatureID, NVSDK_NGX_Parameter* InParameters, NVSDK_NGX_Handle** OutHandle)) {
        return ngxProvider->CreateVulkan1(InDevice, InCmdList, InFeatureID, InParameters, OutHandle);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_D3D12_GetFeatureRequirements, (
        ...
        return ngxProvider->GetFeatureRequirementsD3D12(Adapter, FeatureDiscoveryInfo, RequirementInfo);
    }

    NGX_MAKE_PROXY(NVSDK_NGX_D3D11_GetFeatureRequirements, (
        IDXGIAdapter* Adapter,
        NVSDK_NGX_FeatureDiscoveryInfo* FeatureDiscoveryInfo, NVSDK_NGX_FeatureRequirement* RequirementInfo)) {
        return ngxProvider->GetFeatureRequirementsD3D11(Adapter, FeatureDiscoveryInfo, RequirementInfo);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_VULKAN_GetFeatureRequirements, (
        ...
        NVSDK_NGX_FeatureDiscoveryInfo* FeatureDiscoveryInfo, NVSDK_NGX_FeatureRequirement* RequirementInfo)) {
        return ngxProvider->GetFeatureRequirementsVulkan(instance, device, FeatureDiscoveryInfo, RequirementInfo);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_D3D11_Init, (unsigned long long InApplicationId, const wchar_t* InApplicationDataPath, ID3D11Device* InDevice,
        ...
        return ngxProvider->InitD3D11(InApplicationId, InApplicationDataPath, InDevice, InFeatureInfo, InSDKVersion);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_D3D12_Init, (unsigned long long InApplicationId, const wchar_t* InApplicationDataPath, ID3D12Device* InDevice,
        NVSDK_NGX_FeatureCommonInfo* InFeatureInfo, NVSDK_NGX_Version InSDKVersion)) {
        return ngxProvider->InitD3D12(InApplicationId, InApplicationDataPath, InDevice, InFeatureInfo, InSDKVersion);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_D3D12_Init_Ext,
        ...
        return ngxProvider->InitD3D12Ext(InApplicationId, InApplicationDataPath, InDevice, InSDKVersion, InFeatureInfo);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_D3D11_Init_Ext,
        ...
        return ngxProvider->InitD3D11Ext(InApplicationId, InApplicationDataPath, InDevice, InSDKVersion, InFeatureInfo);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_D3D11_Init_ProjectID,
        ...
        return ngxProvider->InitD3D11ProjectId(InProjectId, InEngineType, InEngineVersion, InApplicationDataPath, InDevice, InSDKVersion, InFeatureInfo);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_D3D12_Init_ProjectID,
        ...
        return ngxProvider->InitD3D12ProjectId(InProjectId, InEngineType, InEngineVersion, InApplicationDataPath, InDevice, InSDKVersion, InFeatureInfo);
    }

    NGX_MAKE_PROXY(NVSDK_NGX_D3D12_Shutdown, ()) {
        return ngxProvider->ShutdownD3D12();
    }

    NGX_MAKE_PROXY(NVSDK_NGX_VULKAN_Shutdown, ()) {
        return ngxProvider->ShutdownVulkan();
    }

    NGX_MAKE_PROXY(NVSDK_NGX_VULKAN_Shutdown1, (::VkDevice InDevice)) {
        return ngxProvider->ShutdownVulkan_1(InDevice);
    }

    NGX_MAKE_PROXY(NVSDK_NGX_D3D11_Shutdown, ()) {
        return ngxProvider->ShutdownD3D11();
    }

    NGX_MAKE_PROXY(NVSDK_NGX_D3D12_Shutdown1, (ID3D12Device* D3DDevice)) {
        return ngxProvider->ShutdownD3D12_1(D3DDevice);
    }

    NGX_MAKE_PROXY(NVSDK_NGX_D3D11_Shutdown1, (ID3D11Device* D3DDevice)) {
        return ngxProvider->ShutdownD3D11_1(D3DDevice);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_D3D11_CreateFeature, (ID3D11DeviceContext* InCmdList, NVSDK_NGX_Feature InFeatureID, NVSDK_NGX_Parameter* InParameters, NVSDK_NGX_Handle** OutHandle)) {
        return ngxProvider->CreateD3D11(InCmdList, InFeatureID, InParameters, OutHandle);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_D3D12_CreateFeature, (ID3D12GraphicsCommandList* InCmdList, NVSDK_NGX_Feature InFeatureID, NVSDK_NGX_Parameter* InParameters, NVSDK_NGX_Handle** OutHandle)) {
        return ngxProvider->CreateD3D12(InCmdList, InFeatureID, InParameters, OutHandle);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_D3D12_EvaluateFeature,
        ...
        return ngxProvider->EvaluateD3D12(InCmdList, InFeatureHandle, InParameters, InCallback);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_VULKAN_GetFeatureInstanceExtensionRequirements, (
        ...
        return ngxProvider->GetFeatureInstanceExtensionRequirementsVulkan(FeatureDiscoveryInfo, OutExtensionCount, OutExtensionProperties);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_D3D11_GetScratchBufferSize, (NVSDK_NGX_Feature InFeatureId, const NVSDK_NGX_Parameter* InParameters, size_t* OutSizeInBytes)) {
        return ngxProvider->GetScratchBufferSizeD3D11(InFeatureId, InParameters, OutSizeInBytes);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_D3D12_GetScratchBufferSize, (NVSDK_NGX_Feature InFeatureId, const NVSDK_NGX_Parameter* InParameters, size_t* OutSizeInBytes)) {
        return ngxProvider->GetScratchBufferSizeD3D12(InFeatureId, InParameters, OutSizeInBytes);
    }
    ...
    NGX_MAKE_PROXY(NVSDK_NGX_VULKAN_GetScratchBufferSize, (NVSDK_NGX_Feature InFeatureId, const NVSDK_NGX_Parameter* InParameters, size_t* OutSizeInBytes)) {
        return ngxProvider->GetScratchBufferSizeVulkan(InFeatureId, InParameters, OutSizeInBytes);
    }

    NGX_MAKE_PROXY(NVSDK_NGX_D3D11_ReleaseFeature, (NVSDK_NGX_Handle* InstanceHandle)) {
        return ngxProvider->ReleaseD3D11(InstanceHandle);
    }

    NGX_MAKE_PROXY(NVSDK_NGX_D3D12_ReleaseFeature, (NVSDK_NGX_Handle* InstanceHandle)) {
        return ngxProvider->ReleaseD3D12(InstanceHandle);
    }

    NGX_MAKE_PROXY(NVSDK_NGX_VULKAN_ReleaseFeature, (NVSDK_NGX_Handle* InstanceHandle)) {
        return ngxProvider->ReleaseVulkan(InstanceHandle);
    }

    struct HookNgx : IHook {
        const std::wstring Name() const override { return L"NGX Provider hooks"; }
        HookPhase   Phase() const override { return HookPhase::ON_DEMAND; }
        int         Priority() const override { return 50; }

        bool CanInstall(Context& ctx, IDetourApi& api) override {
            return
                api.GetModHandleW(L"nvngx.dll") != nullptr && !ctx.ngx.isRealNgxPresent;
        }

        bool Install(Context& ctx, IDetourApi& api) override {
            static auto lastNgx = api.GetModHandleW(L"nvngx.dll");
            auto ngx = api.GetModHandleW(L"nvngx.dll");

            if (lastNgx != nullptr && lastNgx != ngx) {
                LOG_WARNING(L"[NVNGX] NVNGX - new instance found!");
            }
            lastNgx = ngx;
            ...
            LOG_INFO(L"[NVNGX] Initializing NVNGX Provider");
            DetourTxn txn(api);
            ...
            LOG_INFO(L"[NVNGX] NVNGX Provider initialized");
            return true;
        }
    };
}