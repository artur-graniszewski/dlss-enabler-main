...
namespace SettingsMenu
{
    ...
    // =============================================================================
    // Streamline version helpers
    // =============================================================================
    // Version strings come from ctx.streamline.* and may have 2, 3 or 4 components
    // (e.g. L"2.6.10.0", L"2.6.10", L"2.6"). Missing components are treated as 0.
    // The "not loaded" sentinel is L"0.0.0.0" (not an empty string).

    // Parse up to 4 numeric components. Returns false if nothing could be parsed.
    static bool ParseVersionComponents(const std::wstring& ver, int out[4])
    {
        out[0] = out[1] = out[2] = out[3] = 0;
        if (ver.empty())
            return false;
        return swscanf_s(ver.c_str(), L"%d.%d.%d.%d",
            &out[0], &out[1], &out[2], &out[3]) >= 1;
    }

    // True if the version parses to something other than 0.0.0.0
    static bool IsVersionPresent(const std::wstring& ver)
    {
        int v[4];
        if (!ParseVersionComponents(ver, v))
            return false;
        return (v[0] | v[1] | v[2] | v[3]) != 0;
    }

    // True if ver >= reqMajor.reqMinor.reqPatch (build component is ignored)
    static bool IsVersionAtLeast(const std::wstring& ver, int reqMajor, int reqMinor, int reqPatch)
    {
        int v[4];
        if (!ParseVersionComponents(ver, v))
            return false;

        const int req[3] = { reqMajor, reqMinor, reqPatch };
        for (int i = 0; i < 3; ++i)
        {
            if (v[i] != req[i])
                return v[i] > req[i];
    ...
    // Minimum Streamline interposer version required for DFG / MFG Mode Override
    static constexpr int kSlReqMajor = 2;
    static constexpr int kSlReqMinor = 6;
    static constexpr int kSlReqPatch = 10;

    static bool IsStreamlineVersionOk(const std::wstring& ver)
    {
        return IsVersionAtLeast(ver, kSlReqMajor, kSlReqMinor, kSlReqPatch);
    }
    ...
    static void RenderMonitoringBar()
    {
        ...
        // No NVAPI-mode gate: the latency figure is now computed from our own
        // markers and GPU timestamps, so it is just as valid on emulated NVAPI.
    ...
    {
        // Only render when the monitoring bar is visible and DFG Instinct is actually
        // doing something. The Instinct flag alone is not enough: it keeps its stored
        // value while its parent switches turn it off underneath, so the same
        // availability chain the sidebar uses has to be repeated here.
        //   - DFG itself must be on (Instinct is a sub-option of it)
        //   - MFG Mode Override forces DFG off regardless of the stored flags
        //   - Streamline must be loaded and new enough for DFG to exist at all
        {
            const std::wstring& slVer = ctx.streamline.interposerVersion;

            const bool dfgAvailable =
                IsVersionPresent(slVer) &&
                IsStreamlineVersionOk(slVer) &&
                (ctx.nvapi.mfgEnforcedMode == 0);

            const bool instinctActive =
                dfgAvailable &&
                ctx.streamline.isDynamicMfgEnabled &&
                ctx.streamline.isMinDynamicFpsActive;

            if (!ctx.isMonitoringEnabled || !instinctActive)
                return;
        }
    ...
    namespace SidePanel
    {
        ...
        // Content height measured at the end of the last rendered frame; drives the
        // window height on the next one. Zero until the panel has rendered once.
        static float g_MeasuredPanelHeight = 0.0f;

        // Height the panel was actually given this frame, after clamping. Published
        // through SettingsMenu::GetSidePanelHeight() so the "Settings changed" prompt
        // can sit right below the panel instead of recomputing the height itself.
        // Zero whenever the panel is not on screen.
        static float g_ActualPanelHeight = 0.0f;
    ...
    static void RenderSidePanel()
    {
        ...
        {
            SidePanel::g_ActualPanelHeight = 0.0f;
            return;
        }
        ...
        // Panel dimensions - height follows the content, width stays animated.
        // Sections hide whole groups of controls (SSRT off, DFG off, ...), so a fixed
        // height leaves a few hundred px of empty background.
        //
        // The height comes from measuring the content at the end of the PREVIOUS frame
        // (g_MeasuredPanelHeight, written just before End()) rather than from
        // AlwaysAutoResize, which this ImGui build does not honour here. One frame of
        // lag is invisible because the height only changes on user action.
        ...
        // Fallback for the very first frame, before anything has been measured.
        float desiredPanelHeight = (SidePanel::g_MeasuredPanelHeight > 1.0f)
            ? SidePanel::g_MeasuredPanelHeight
            : S(750.0f);
        if (desiredPanelHeight > maxPanelHeight)
            desiredPanelHeight = maxPanelHeight;

        SidePanel::g_ActualPanelHeight = desiredPanelHeight;

        // Position: top-left with 10px margin
        UxImVec2 panelPos(panelMargin, panelMargin);

        UxImGui::SetNextWindowPos(panelPos);
        UxImGui::SetNextWindowSize(UxImVec2(panelWidth, desiredPanelHeight));
        ...
        // Allow vertical scrolling (needed once the content exceeds maxPanelHeight)
        UxImGuiWindowFlags flags = UxImGuiWindowFlags_NoTitleBar |
            UxImGuiWindowFlags_NoResize |
            UxImGuiWindowFlags_NoMove |
            UxImGuiWindowFlags_NoCollapse;
        ...
        if (UxImGui::Begin("##SidePanel", nullptr, flags))
        {
            ...
            // =====================================================
            // Title row: "DLSS Enabler" left, version right
            // =====================================================
            {
                ...
                const UxImVec4 titleColor = UxImVec4(0.0f, 0.78f, 0.71f, 1.0f);  // Teal, matches left accent line
                std::string versionLine = "v" + s_dlssEnablerVersion;

                UxImGui::TextColored(titleColor, "DLSS Enabler");

                // Right-align the version against the panel's right padding
                float versionWidth = UxImGui::CalcTextSize(versionLine.c_str()).x;
                float versionX = panelWidth - S(15.0f) - versionWidth;
                if (versionX < S(15.0f)) versionX = S(15.0f);

                UxImGui::SameLine(versionX);
                UxImGui::TextColored(titleColor, "%s", versionLine.c_str());
            }

            UxImGui::Dummy(UxImVec2(0, S(14.0f)));
            ...
            // Slider paired with a keyboard-editable number box on its right.
            // Shorter than DrawSlider: the slider gives up exactly the width the box needs,
            // so the row still spans panelWidth - 2*15 like every other control.
            // Both widths come from CalcTextSize/S(), so the pair scales with g_UiScale
            // instead of drifting apart at non-1080p resolutions.
            //
            // The 4-byte buffer is what caps typing at 3 digits, so a maxValue above 999
            // would be reachable by the slider but not from the keyboard.
            // Hard character filter for the number box: only 0-9 survives.
            // CharsDecimal is NOT enough - it also lets through . + - * / , which the
            // digit parser below would silently mangle ("3.5" would commit as 35).
            // Captureless, so it converts to the plain function pointer InputText wants.
            auto DigitsOnlyFilter = [](UxImGuiInputTextCallbackData* data) -> int
                {
                    return (data->EventChar < '0' || data->EventChar > '9') ? 1 : 0;
                };

            // labelColor defaults to the toggle-row colour; DFG-style rows pass the dimmer
            // grey they already use, so the component can serve both conventions unchanged.
            auto DrawSliderInput = [&](const char* label, int* value, int minValue, int maxValue,
                bool enabled = true,
                UxImVec4 accent = UxImVec4(0.0f, 0.75f, 0.7f, 1.0f),
                UxImVec4 accentActive = UxImVec4(0.0f, 0.85f, 0.8f, 1.0f),
                const char* valueFormat = "%d",
                UxImVec4 labelColor = UxImVec4(0.85f, 0.85f, 0.85f, 1.0f)) -> bool
                {
                    bool changed = false;
                    ...
                    UxImGui::SetCursorPosX(S(15.0f));
                    UxImGui::PushStyleColor(UxImGuiCol_Text, enabled ? labelColor : textDisabled);
                    UxImGui::Text("%s", label);
                    UxImGui::PopStyleColor();

                    const float inputWidth = UxImGui::CalcTextSize("000").x + S(16.0f);
                    const float gap = S(8.0f);
                    float sliderWidth = panelWidth - S(30.0f) - inputWidth - gap;
                    if (sliderWidth < S(40.0f))
                        sliderWidth = S(40.0f);

                    UxImGui::SetCursorPosX(S(15.0f));
                    UxImGui::PushStyleColor(UxImGuiCol_FrameBg, enabled ? UxImVec4(0.1f, 0.12f, 0.15f, 1.0f) : sliderBgDisabled);
                    UxImGui::PushStyleColor(UxImGuiCol_SliderGrab, enabled ? accent : UxImVec4(0.2f, 0.2f, 0.22f, 1.0f));
                    UxImGui::PushStyleColor(UxImGuiCol_SliderGrabActive, enabled ? accentActive : UxImVec4(0.2f, 0.2f, 0.22f, 1.0f));
                    UxImGui::PushStyleColor(UxImGuiCol_Text, enabled ? textEnabled : textDisabled);
                    UxImGui::PushStyleVar(UxImGuiStyleVar_FrameRounding, S(4.0f));
                    UxImGui::PushStyleVar(UxImGuiStyleVar_GrabRounding, S(4.0f));

                    UxImGui::BeginDisabled(!enabled);

                    UxImGui::PushItemWidth(sliderWidth);
                    if (UxImGui::SliderInt("##sliderPart", value, minValue, maxValue, valueFormat))
                        changed = true;
                    UxImGui::PopItemWidth();

                    UxImGui::SameLine(0.0f, gap);

                    // 4 bytes = 3 digits + NUL. ImGui only reads this on activation and
                    // keeps its own state while the box is focused, so refreshing it every
                    // frame does not fight the user's typing.
                    char buf[4] = { 0 };
                    {
                        const std::string cur = std::to_string(*value);
                        const size_t n = cur.size() < 3 ? cur.size() : 3;
                        for (size_t i = 0; i < n; ++i)
                            buf[i] = cur[i];
                    }

                    // ---- per-instance state --------------------------------------------
                    // Keyed by the label pointer (string literals are stable for the lifetime
                    // of the process). Looked up BEFORE the box is drawn, because the
                    // out-of-range tint is decided from what was typed last frame.
                    // If more than kMaxTrackedInputs instances exist, the extra ones simply
                    // lose the timer and the tint, and keep working through Enter / focus loss.
                    const ULONGLONG kInputIdleCommitMs = 2000;
                    const int kMaxTrackedInputs = 8;
                    ...
                    static InputIdleState s_idle[kMaxTrackedInputs] = {};

                    InputIdleState* idle = nullptr;
                    for (int i = 0; i < kMaxTrackedInputs; ++i)
                    {
                        if (s_idle[i].key == label) { idle = &s_idle[i]; break; }
                        if (s_idle[i].key == nullptr) { s_idle[i].key = label; idle = &s_idle[i]; break; }
                    }

                    // Typed text is never overwritten - an out-of-range entry is only flagged
                    // by tinting the box. FrameBgHovered/Active are pushed too, otherwise the
                    // tint would vanish the moment the box has focus, which is exactly when
                    // the user is looking at it.
                    const bool tintInvalid = enabled && idle != nullptr && idle->outOfRange;
                    if (tintInvalid)
                    {
                        UxImGui::PushStyleColor(UxImGuiCol_FrameBg, UxImVec4(0.32f, 0.11f, 0.13f, 1.0f));
                        UxImGui::PushStyleColor(UxImGuiCol_FrameBgHovered, UxImVec4(0.38f, 0.14f, 0.16f, 1.0f));
                        UxImGui::PushStyleColor(UxImGuiCol_FrameBgActive, UxImVec4(0.38f, 0.14f, 0.16f, 1.0f));
                        UxImGui::PushStyleColor(UxImGuiCol_Text, UxImVec4(1.0f, 0.60f, 0.60f, 1.0f));
                    }

                    UxImGui::PushItemWidth(inputWidth);
                    const bool submitted = UxImGui::InputText("##inputPart", buf, sizeof(buf),
                        UxImGuiInputTextFlags_CallbackCharFilter |
                        UxImGuiInputTextFlags_AutoSelectAll |
                        UxImGuiInputTextFlags_EnterReturnsTrue,
                        DigitsOnlyFilter);
                    const bool finished = UxImGui::IsItemDeactivatedAfterEdit();
                    const bool boxActive = UxImGui::IsItemActive();
                    UxImGui::PopItemWidth();

                    if (tintInvalid)
                        UxImGui::PopStyleColor(4);

                    if (tintInvalid && UxImGui::IsItemHovered())
                    {
                        UxImGui::BeginTooltip();
                        UxImGui::Text("Value outside %d-%d, will be clamped", minValue, maxValue);
                        UxImGui::EndTooltip();
                    }

                    // ---- idle debounce -------------------------------------------------
                    // Typing "300" must not commit "3" then "30" on the way. Instead each
                    // keystroke re-arms a timer and the value lands once the user stops
                    // typing for kInputIdleCommitMs. Enter and focus loss still commit
                    // immediately - the timer is a fallback for people who type and look away.
                    ...
                    if (idle != nullptr)
                    {
                        ...
                        const bool textDiffers =
                            buf[0] != idle->lastText[0] || buf[1] != idle->lastText[1] ||
                            buf[2] != idle->lastText[2] || buf[3] != idle->lastText[3];
                        ...
                        if (boxActive)
                        {
                            ...
                            {
                                for (int i = 0; i < 4; ++i)
                                    idle->lastText[i] = buf[i];
                                idle->lastEditTime = now;
                                idle->armed = true;
                            }
                            else if (idle->armed && (now - idle->lastEditTime) >= kInputIdleCommitMs)
                            {
                                idleElapsed = true;
                                idle->armed = false;
                            }

                            // Verdict for the NEXT frame's tint. Only digits can be in the
                            // buffer, so a plain parse is enough.
                            int peek = 0;
                            for (const char* c = buf; *c != '\0'; ++c)
                                peek = peek * 10 + (*c - '0');

                            idle->outOfRange = (buf[0] != '\0') && (peek < minValue || peek > maxValue);
                        }
                        else
                        {
                            // Not being edited - keep the snapshot in sync so a slider drag
                            // does not look like typing the next time the box gets focus,
                            // and drop the tint, since the box now mirrors the live value.
                            for (int i = 0; i < 4; ++i)
                                idle->lastText[i] = buf[i];
                            idle->armed = false;
                            idle->outOfRange = false;
                        }
                    }
                    // --------------------------------------------------------------------

                    // Commit on Enter, on focus loss, or after the idle timer - never on a
                    // single keystroke, otherwise "3" on the way to "300" would be clamped
                    // up to minValue under the user.
                    //
                    // An empty box is treated as "no change" rather than 0 -> minValue, so
                    // clearing the field and tabbing away leaves the current value alone.
                    if (enabled && buf[0] != '\0' && (submitted || finished || idleElapsed))
                    {
                        int parsed = 0;
                        for (const char* c = buf; *c != '\0'; ++c)
            ...
            // One-shot sanity default for the FPS limit.
            // This file is not the owner of ctx.reflex.desiredFpsLimit - it only reads and
            // writes it - so a value outside the slider range means nothing sensible was
            // loaded from INI. Landing on 60 beats the 30 the slider would otherwise snap to
            // on first touch. Runs once per process, so a value loaded later from INI wins.
            // TODO: the real default belongs in whichever layer owns ctx.reflex / INI loading.
            {
                static bool s_fpsLimitDefaultApplied = false;
                if (!s_fpsLimitDefaultApplied)
                {
                    s_fpsLimitDefaultApplied = true;
                    if (ctx.reflex.desiredFpsLimit < 30 || ctx.reflex.desiredFpsLimit > 360)
                        ctx.reflex.desiredFpsLimit = 60;
                }
            }
            ...
            // TEST BED for DrawSliderInput - first user of the slider + number box pair
            //
            // Hidden entirely unless FPS Limit is ON. Still greyed out on top of that when
            // Overdrive is active, because Overdrive owns the limit and the toggle above
            // can stay ON while Overdrive drives the actual value.
            if (ctx.reflex.isFpsLimitEnabled)
            {
                UxImGui::Dummy(UxImVec2(0, 4));

                DrawSliderInput("Limit", &ctx.reflex.desiredFpsLimit, 30, 360,
            ...
            // =====================================================
            // MULTI FRAME GEN Section
            // =====================================================
            UxImGui::SetCursorPosX(S(15.0f));
            UxImGui::PushStyleColor(UxImGuiCol_Text, UxImVec4(0.5f, 0.55f, 0.6f, 1.0f));
            UxImGui::Text("MULTI FRAME GEN");
            UxImGui::PopStyleColor();
            ...
            //
            // Requires Streamline >= 2.6.10 for any override; x5/x6 needs sl.dlss_g >= 2.11
            {
                // Requires Streamline interposer >= 2.6.10; x5/x6 additionally needs sl.dlss_g >= 2.11
                // ctx.streamline.interposerVersion is e.g. L"2.6.10.0", default L"0.0.0.0"
                const std::wstring& slVer = ctx.streamline.interposerVersion;
                const bool streamlineLoaded = IsVersionPresent(slVer);
                const bool streamlineVersionOk = IsStreamlineVersionOk(slVer);

                // sl.dlss_g >= 2.11 (5X/6X capable)
                const bool dlssg211 = IsVersionAtLeast(ctx.streamline.dlssgVersion, 2, 11, 0);
                ...
                else
                {
                    ...
                    if (UxImGui::IsItemHovered())
                    {
                        ...
                        if (!streamlineLoaded)
                            ...
                            UxImGui::Text("Requires Streamline 2.6.10 or newer");
                        UxImGui::EndTooltip();
                    }
                    UxImGui::PopID();
            ...
            // =====================================================
            // DYNAMIC FRAME GEN Section
            // =====================================================
            UxImGui::SetCursorPosX(S(15.0f));
            UxImGui::PushStyleColor(UxImGuiCol_Text, UxImVec4(0.5f, 0.55f, 0.6f, 1.0f));
            UxImGui::Text("DYNAMIC FRAME GEN");
            UxImGui::PopStyleColor();
            ...
            // Available when Streamline is loaded with version >= 2.6.10
            const std::wstring& slVer = ctx.streamline.interposerVersion;
            const bool streamlineLoaded = IsVersionPresent(slVer);
            const bool streamlineVersionOk = IsStreamlineVersionOk(slVer);
            const bool dmfgAvailable = streamlineLoaded && streamlineVersionOk && !mfgOverrideActive;
            ...
            // Remaining DFG controls are hidden entirely unless the toggle above is ON.
            // Covers both of its non-ON states: user-set OFF, and N/A when unavailable.
            const bool dfgVisible = dmfgAvailable && ctx.streamline.isDynamicMfgEnabled;
            if (dfgVisible)
            {
                UxImGui::Dummy(UxImVec2(0, 4));
                ...
                // Hidden entirely unless DFG Instinct is ON - covers both of its other
                // states: user-set OFF, and N/A when unavailable.
                if (ctx.streamline.isMinDynamicFpsActive)
                {
                    ...
                    DrawSliderInput("Minimum FPS", &minFpsInt, 30, 240, true,
                        UxImVec4(0.55f, 0.35f, 0.0f, 1.0f),   // Amber, matches DFG Instinct
                        UxImVec4(0.65f, 0.45f, 0.0f, 1.0f),
                        "%d FPS",
                        UxImVec4(0.65f, 0.7f, 0.75f, 1.0f));
                ...
                // Changes here auto-compute thresholds: threshold_N = ceil(targetFPS / N)
                {
                    ...
                    DrawSliderInput("Target FPS (auto thresholds)", &ctx.streamline.dfgTargetFps, 30, 300,
                        sliderActive,
                        UxImVec4(0.0f, 0.6f, 0.55f, 1.0f),
                        UxImVec4(0.0f, 0.7f, 0.65f, 1.0f),
                        "%d FPS",
            ...
            // Remaining ray tracing controls are hidden entirely unless SSRT is ON.
            // Covers both states of the row above: user-set OFF, and N/A when unavailable.
            if (rtEnabled)
            {
                UxImGui::Dummy(UxImVec2(0, 4));
            ...
            //
            // HIDDEN - no longer used. Kept compiled so it does not silently rot.
            // CANDIDATE FOR PHYSICAL REMOVAL: delete this whole block together with
            // kShowInitOptiscalerEarly once the decision is final.
            constexpr bool kShowInitOptiscalerEarly = false;
            if (kShowInitOptiscalerEarly)
            {
                UxImGui::Dummy(UxImVec2(0, 6));
            ...
            // These control bits in ctx.flags
            {
                ...
                // Debug Overlays label on its own line
                UxImGui::SetCursorPosX(S(15.0f));
                UxImGui::PushStyleColor(UxImGuiCol_Text, fgActive ? textEnabled : textDisabled);
                UxImGui::Text("Debug Overlays");
                UxImGui::PopStyleColor();
            ...
            // Measure what the content actually needed, for the next frame's window size.
            // GetCursorPosY() is window-local and already compensates for scrolling, so it
            // is the full content height even when the panel is scrolled.
            SidePanel::g_MeasuredPanelHeight = UxImGui::GetCursorPosY();
        }
        UxImGui::End();
    ...
    {
        return SidePanel::g_ActualPanelHeight;
    }
