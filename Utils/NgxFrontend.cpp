...
namespace NGX
{
	...
	namespace
	{
			...
			struct SubframeBucket {
				...
				void PushFrame(const FfxProfFrameReport& r) {
					...
					for (uint32_t i = 0; i < r.passCount; ++i) {
						...
						passes[std::wstring(r.passes[i].label)].Push(ms);
		...
		//   [MFG-PROF 100f] subframe 1/3  TOTAL: avg=X.XXX med=X.XXX p99=X.XXX  (29 passes)
		//     OF: avg=X.XXX med=X.XXX p99=X.XXX  (N passes)
		//         Opticalflow_Luma           avg=0.082  med=0.081  p99=0.104
		//         OF Search Level 3          avg=0.421  med=0.418  p99=0.491
		//         ...
		//     FI: avg=X.XXX med=X.XXX p99=X.XXX  (M passes)
		//         INTERPOLATION              avg=2.143  med=2.131  p99=2.294
		//         DISOCCLUSION_MASK          avg=0.312  med=0.308  p99=0.341
		//         ...
		static void EmitSubframeReport(int32_t subframeIdx, int32_t subframeCount,
			const MfgAgg::SubframeBucket& bucket)
		{
			...
			wchar_t header[224];
			swprintf_s(header,
				L"[MFG-PROF %zuf] subframe %d/%d  TOTAL: avg=%.3fms  med=%.3fms  p99=%.3fms  (%zu passes)",
				MfgAgg::kWindowSize,
			...
			auto emitGroup = [&](const wchar_t* name,
				...
				const std::vector<Row>& rows)
				{
					...
					for (const Row& r : rows) {
						wchar_t line[256];
						swprintf_s(line,
							L"      %-36ls  avg=%7.3f  med=%7.3f  p99=%7.3f",
							r.label->c_str(),
