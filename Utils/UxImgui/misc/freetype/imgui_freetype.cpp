...
//  2025/XX/XX: refactored for the new UxImFontLoader architecture, and UxImGuiBackendFlags_RendererHasTextures support.
...
//  2023/11/13: added support for UxImFontConfig::RasterizationDensity field for scaling render density without scaling metrics.
...
//  2021/03/05: added UxImGuiFreeTypeBuilderFlags_Bitmap to load bitmap glyphs.
//  2021/03/02: set 'atlas->TexPixelsUseColors = true' to help some backends with deciding of a preferred texture format.
//  2021/01/28: added support for color-layered glyphs via UxImGuiFreeTypeBuilderFlags_LoadColor (require Freetype 2.10+).
//  2021/01/26: simplified integration by using '#define IMGUI_ENABLE_FREETYPE'. renamed UxImGuiFreeType::XXX flags to UxImGuiFreeTypeBuilderFlags_XXX for consistency with other API. removed UxImGuiFreeType::BuildFontAtlas().
...
//  2018/06/08: added support for UxImFontConfig::GlyphMinAdvanceX, GlyphMaxAdvanceX.
//  2018/02/04: moved to main imgui repository (away from http://www.github.com/ocornut/imgui_club)
//  2018/01/22: fix for addition of UxImFontAtlas::TexUvscale member.
...
//  2017/08/26: cleanup, optimizations, support for UxImFontConfig::RasterizerFlags, UxImFontConfig::RasterizerMultiply.
//  2017/08/16: imported from https://github.com/Vuhdo/imgui_freetype into http://www.github.com/ocornut/imgui_club, updated for latest changes in UxImFontAtlas, minor tweaks.
...
#include "../../imgui_internal.h"     // UxImMin,UxImMax,UxImFontAtlasBuild*,
#include <stdint.h>
#include <ft2build.h>
...
static void* UxImGuiFreeTypeDefaultAllocFunc(size_t size, void* user_data) { IM_UNUSED(user_data); return IM_ALLOC(size); }
static void  UxImGuiFreeTypeDefaultFreeFunc(void* ptr, void* user_data) { IM_UNUSED(user_data); IM_FREE(ptr); }

// Current memory allocators
static void* (*GImGuiFreeTypeAllocFunc)(size_t size, void* user_data) = UxImGuiFreeTypeDefaultAllocFunc;
static void  (*GImGuiFreeTypeFreeFunc)(void* ptr, void* user_data) = UxImGuiFreeTypeDefaultFreeFunc;
static void* GImGuiFreeTypeAllocatorUserData = nullptr;

// Lunasvg support
#ifdef IMGUI_ENABLE_FREETYPE_LUNASVG
static FT_Error UxImGuiLunasvgPortInit(FT_Pointer* state);
static void     UxImGuiLunasvgPortFree(FT_Pointer* state);
static FT_Error UxImGuiLunasvgPortRender(FT_GlyphSlot slot, FT_Pointer* _state);
static FT_Error UxImGuiLunasvgPortPresetSlot(FT_GlyphSlot slot, FT_Bool cache, FT_Pointer* _state);
#endif
...
// Stored in UxImFontAtlas::FontLoaderData. ALLOCATED BY US.
struct UxImGui_ImplFreeType_Data
{
    FT_Library                      Library;
    FT_MemoryRec_                   MemoryManager;
    UxImGui_ImplFreeType_Data()       { memset((void*)this, 0, sizeof(*this)); }
};

// Stored in UxImFontConfig::FontLoaderData. ALLOCATED BY US.
struct UxImGui_ImplFreeType_FontSrcData
{
    // Initialize from an external data buffer. Doesn't copy data, and you must ensure it stays valid up to this object lifetime.
    bool                            InitFont(FT_Library ft_library, UxImFontConfig* src, UxImGuiFreeTypeBuilderFlags extra_user_flags);
    void                            CloseFont();
    UxImGui_ImplFreeType_FontSrcData()   { memset((void*)this, 0, sizeof(*this)); }
    ~UxImGui_ImplFreeType_FontSrcData()  { CloseFont(); }

    // Members
    FT_Face                         FtFace;
    UxImGuiFreeTypeBuilderFlags       UserFlags;          // = UxImFontConfig::FontBuilderFlags
    FT_Int32                        LoadFlags;
    UxImFontBaked*                    BakedLastActivated;
};

// Stored in UxImFontBaked::FontLoaderDatas: pointer to SourcesCount instances of this. ALLOCATED BY CORE.
struct UxImGui_ImplFreeType_FontSrcBakedData
{
    FT_Size     FtSize;             // This represent a FT_Face with a given size.
    UxImGui_ImplFreeType_FontSrcBakedData() { memset((void*)this, 0, sizeof(*this)); }
};

bool UxImGui_ImplFreeType_FontSrcData::InitFont(FT_Library ft_library, UxImFontConfig* src, UxImGuiFreeTypeBuilderFlags extra_font_builder_flags)
{
    ...
    UserFlags = (UxImGuiFreeTypeBuilderFlags)(src->FontBuilderFlags | extra_font_builder_flags);

    LoadFlags = 0;
    if ((UserFlags & UxImGuiFreeTypeBuilderFlags_Bitmap) == 0)
        LoadFlags |= FT_LOAD_NO_BITMAP;

    if (UserFlags & UxImGuiFreeTypeBuilderFlags_NoHinting)
        LoadFlags |= FT_LOAD_NO_HINTING;
    else
        src->PixelSnapH = true; // FIXME: A bit weird to do this this way.

    if (UserFlags & UxImGuiFreeTypeBuilderFlags_NoAutoHint)
        LoadFlags |= FT_LOAD_NO_AUTOHINT;
    if (UserFlags & UxImGuiFreeTypeBuilderFlags_ForceAutoHint)
        LoadFlags |= FT_LOAD_FORCE_AUTOHINT;
    if (UserFlags & UxImGuiFreeTypeBuilderFlags_LightHinting)
        LoadFlags |= FT_LOAD_TARGET_LIGHT;
    else if (UserFlags & UxImGuiFreeTypeBuilderFlags_MonoHinting)
        LoadFlags |= FT_LOAD_TARGET_MONO;
    else
        LoadFlags |= FT_LOAD_TARGET_NORMAL;

    if (UserFlags & UxImGuiFreeTypeBuilderFlags_LoadColor)
        LoadFlags |= FT_LOAD_COLOR;

    return true;
}

void UxImGui_ImplFreeType_FontSrcData::CloseFont()
{
    if (FtFace)
    {
        FT_Done_Face(FtFace);
...
static const FT_Glyph_Metrics* UxImGui_ImplFreeType_LoadGlyph(UxImGui_ImplFreeType_FontSrcData* src_data, uint32_t codepoint)
{
    ...
    if (src_data->UserFlags & UxImGuiFreeTypeBuilderFlags_Bold)
        FT_GlyphSlot_Embolden(slot);
    if (src_data->UserFlags & UxImGuiFreeTypeBuilderFlags_Oblique)
    {
...
static void UxImGui_ImplFreeType_BlitGlyph(const FT_Bitmap* ft_bitmap, uint32_t* dst, uint32_t dst_pitch)
{
    ...
    case FT_PIXEL_MODE_BGRA:
        {
            ...
            #define DE_MULTIPLY(color, alpha) UxImMin((UxImU32)(255.0f * (float)color / (float)(alpha + FLT_MIN) + 0.5f), 255u)
            for (uint32_t y = 0; y < h; y++, src += src_pitch, dst += dst_pitch)
                for (uint32_t x = 0; x < w; x++)
                {
...
bool UxImGui_ImplFreeType_LoaderInit(UxImFontAtlas* atlas)
{
    IM_ASSERT(atlas->FontLoaderData == NULL);
    UxImGui_ImplFreeType_Data* bd = IM_NEW(UxImGui_ImplFreeType_Data)();
    ...
    SVG_RendererHooks hooks = { UxImGuiLunasvgPortInit, UxImGuiLunasvgPortFree, UxImGuiLunasvgPortRender, UxImGuiLunasvgPortPresetSlot };
...
void UxImGui_ImplFreeType_LoaderShutdown(UxImFontAtlas* atlas)
{
    UxImGui_ImplFreeType_Data* bd = (UxImGui_ImplFreeType_Data*)atlas->FontLoaderData;
    IM_ASSERT(bd != NULL);
...
bool UxImGui_ImplFreeType_FontSrcInit(UxImFontAtlas* atlas, UxImFontConfig* src)
{
    UxImGui_ImplFreeType_Data* bd = (UxImGui_ImplFreeType_Data*)atlas->FontLoaderData;
    UxImGui_ImplFreeType_FontSrcData* bd_font_data = IM_NEW(UxImGui_ImplFreeType_FontSrcData);
    IM_ASSERT(src->FontLoaderData == NULL);
    src->FontLoaderData = bd_font_data;

    if (!bd_font_data->InitFont(bd->Library, src, (UxImGuiFreeTypeBuilderFlags)atlas->FontBuilderFlags))
    {
...
void UxImGui_ImplFreeType_FontSrcDestroy(UxImFontAtlas* atlas, UxImFontConfig* src)
{
    IM_UNUSED(atlas);
    UxImGui_ImplFreeType_FontSrcData* bd_font_data = (UxImGui_ImplFreeType_FontSrcData*)src->FontLoaderData;
    IM_DELETE(bd_font_data);
    src->FontLoaderData = NULL;
}

bool UxImGui_ImplFreeType_FontBakedInit(UxImFontAtlas* atlas, UxImFontConfig* src, UxImFontBaked* baked, void* loader_data_for_baked_src)
{
    IM_UNUSED(atlas);
    ...
    UxImGui_ImplFreeType_FontSrcBakedData* bd_baked_data = (UxImGui_ImplFreeType_FontSrcBakedData*)loader_data_for_baked_src;
    IM_ASSERT(bd_baked_data != NULL);
    IM_PLACEMENT_NEW(bd_baked_data) UxImGui_ImplFreeType_FontSrcBakedData();

    FT_New_Size(bd_font_data->FtFace, &bd_baked_data->FtSize);
    FT_Activate_Size(bd_baked_data->FtSize);

    // Vuhdo 2017: "I'm not sure how to deal with font sizes properly. As far as I understand, currently UxImGui assumes that the 'pixel_height'
    ...
    req.type = (bd_font_data->UserFlags & UxImGuiFreeTypeBuilderFlags_Bitmap) ? FT_SIZE_REQUEST_TYPE_NOMINAL : FT_SIZE_REQUEST_TYPE_REAL_DIM;
    req.width = 0;
...
void UxImGui_ImplFreeType_FontBakedDestroy(UxImFontAtlas* atlas, UxImFontConfig* src, UxImFontBaked* baked, void* loader_data_for_baked_src)
{
    IM_UNUSED(atlas);
    IM_UNUSED(baked);
    IM_UNUSED(src);
    ...
    bd_baked_data->~UxImGui_ImplFreeType_FontSrcBakedData(); // ~IM_PLACEMENT_DELETE()
}

bool UxImGui_ImplFreeType_FontBakedLoadGlyph(UxImFontAtlas* atlas, UxImFontConfig* src, UxImFontBaked* baked, void* loader_data_for_baked_src, UxImWchar codepoint, UxImFontGlyph* out_glyph)
{
    UxImGui_ImplFreeType_FontSrcData* bd_font_data = (UxImGui_ImplFreeType_FontSrcData*)src->FontLoaderData;
    ...
    const FT_Glyph_Metrics* metrics = UxImGui_ImplFreeType_LoadGlyph(bd_font_data, codepoint);
    if (metrics == NULL)
        return false;
    ...
    FT_Render_Mode render_mode = (bd_font_data->UserFlags & UxImGuiFreeTypeBuilderFlags_Monochrome) ? FT_RENDER_MODE_MONO : FT_RENDER_MODE_NORMAL;
    ...
    if (is_visible)
    {
        ...
        UxImGui_ImplFreeType_BlitGlyph(ft_bitmap, temp_buffer, w);
        ...
        UxImFontAtlasBakedSetFontGlyphBitmap(atlas, baked, src, out_glyph, r, (const unsigned char*)temp_buffer, UxImTextureFormat_RGBA32, w * 4);
    }

    return true;
}

bool UxImGui_ImplFreetype_FontSrcContainsGlyph(UxImFontAtlas* atlas, UxImFontConfig* src, UxImWchar codepoint)
{
    IM_UNUSED(atlas);
    UxImGui_ImplFreeType_FontSrcData* bd_font_data = (UxImGui_ImplFreeType_FontSrcData*)src->FontLoaderData;
...
const UxImFontLoader* UxImGuiFreeType::GetFontLoader()
{
    static UxImFontLoader loader;
    loader.Name = "FreeType";
    loader.LoaderInit = UxImGui_ImplFreeType_LoaderInit;
    loader.LoaderShutdown = UxImGui_ImplFreeType_LoaderShutdown;
    loader.FontSrcInit = UxImGui_ImplFreeType_FontSrcInit;
    loader.FontSrcDestroy = UxImGui_ImplFreeType_FontSrcDestroy;
    loader.FontSrcContainsGlyph = UxImGui_ImplFreetype_FontSrcContainsGlyph;
    loader.FontBakedInit = UxImGui_ImplFreeType_FontBakedInit;
    loader.FontBakedDestroy = UxImGui_ImplFreeType_FontBakedDestroy;
    loader.FontBakedLoadGlyph = UxImGui_ImplFreeType_FontBakedLoadGlyph;
    loader.FontBakedSrcLoaderDataSize = sizeof(UxImGui_ImplFreeType_FontSrcBakedData);
    return &loader;
}

void UxImGuiFreeType::SetAllocatorFunctions(void* (*alloc_func)(size_t sz, void* user_data), void (*free_func)(void* ptr, void* user_data), void* user_data)
{
...
bool UxImGuiFreeType::DebugEditFontBuilderFlags(unsigned int* p_font_loader_flags)
{
    bool edited = false;
    edited |= UxImGui::CheckboxFlags("NoHinting",    p_font_loader_flags, UxImGuiFreeTypeBuilderFlags_NoHinting);
    edited |= UxImGui::CheckboxFlags("NoAutoHint",   p_font_loader_flags, UxImGuiFreeTypeBuilderFlags_NoAutoHint);
    edited |= UxImGui::CheckboxFlags("ForceAutoHint",p_font_loader_flags, UxImGuiFreeTypeBuilderFlags_ForceAutoHint);
    edited |= UxImGui::CheckboxFlags("LightHinting", p_font_loader_flags, UxImGuiFreeTypeBuilderFlags_LightHinting);
    edited |= UxImGui::CheckboxFlags("MonoHinting",  p_font_loader_flags, UxImGuiFreeTypeBuilderFlags_MonoHinting);
    edited |= UxImGui::CheckboxFlags("Bold",         p_font_loader_flags, UxImGuiFreeTypeBuilderFlags_Bold);
    edited |= UxImGui::CheckboxFlags("Oblique",      p_font_loader_flags, UxImGuiFreeTypeBuilderFlags_Oblique);
    edited |= UxImGui::CheckboxFlags("Monochrome",   p_font_loader_flags, UxImGuiFreeTypeBuilderFlags_Monochrome);
    edited |= UxImGui::CheckboxFlags("LoadColor",    p_font_loader_flags, UxImGuiFreeTypeBuilderFlags_LoadColor);
    edited |= UxImGui::CheckboxFlags("Bitmap",       p_font_loader_flags, UxImGuiFreeTypeBuilderFlags_Bitmap);
    return edited;
}
...
static FT_Error UxImGuiLunasvgPortInit(FT_Pointer* _state)
{
    *_state = IM_NEW(LunasvgPortState)();
    return FT_Err_Ok;
}

static void UxImGuiLunasvgPortFree(FT_Pointer* _state)
{
...
static FT_Error UxImGuiLunasvgPortRender(FT_GlyphSlot slot, FT_Pointer* _state)
{
    ...
    // If there was an error while loading the svg in UxImGuiLunasvgPortPresetSlot(), the renderer hook still get called, so just returns the error.
...
static FT_Error UxImGuiLunasvgPortPresetSlot(FT_GlyphSlot slot, FT_Bool cache, FT_Pointer* _state)
{
    ...
    // This function is called twice, once in the FT_Load_Glyph() and another right before UxImGuiLunasvgPortRender().
    ...
    slot->bitmap.rows = (unsigned int)(UxImCeil((float)box.h));
    slot->bitmap.width = (unsigned int)(UxImCeil((float)box.w));
